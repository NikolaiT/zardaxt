...
def main():
    db = load_database()
    data = get_data()
    training, testing = get_learning_data(data, threshold=0.9)
    numFalse = 0
    numCorrect = 0
    ...
    for fp in testing:
        ...
        if not same:
            ...
            numCorrect += 1
    print(
        f'Ratio of correctly classified without ip_total_length and tcp_off: {numFalse}/{numCorrect}')

    numFalse = 0
    numCorrect = 0
    for fp in testing:
        score = score_fp(fp, db, use_ip_total_length=True, use_tcp_off=True)
        highest_os = max(score, key=score.get)
        same = same_os(highest_os, fp['os'])
        if not same:
            numFalse += 1
            ...
            numCorrect += 1
    print(
        f'Ratio of correctly classified with ip_total_length and tcp_off: {numFalse}/{numCorrect}')
